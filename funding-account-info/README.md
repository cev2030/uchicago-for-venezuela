# Donate — UChicago For Venezuela

All donations support earthquake relief for Venezuelan families and student
volunteers: buying medical supplies, food, rescue equipment, and logistics
(see [`../supplies-needed/`](../supplies-needed/) for the current needs list).

Donations are received by **1001 Ideas, INC**, a U.S.-based 501(c)(3)
nonprofit organization. **Donations from U.S. taxpayers are tax-deductible to
the extent allowed by law.** (See [Tax-Deductibility](#tax-deductibility--501c3-information) below.)

---

## How to donate

### Option 1 — Zelle (preferred)

Zelle is the fastest, lowest-friction way to give — no fees, and the funds
arrive almost instantly. Most U.S. banking apps have Zelle built in.

1. Open your bank's mobile app or website and find **Zelle** (often under
   "Send money" / "Transfers").
2. Add a recipient using this email:

   > **Zelle email:** `finanzasmiliun@gmail.com`

3. Confirm the recipient name shows as **1001 Ideas, INC** (or the
   associated account holder) before sending.
4. Enter the amount and, in the memo/note field, write **"Venezuela
   earthquake relief"** so we can track and acknowledge your gift.
5. Send. Take a screenshot of the confirmation — you'll want it for your
   tax records (see below).

> 💡 If you'd like a donation receipt, email us your name, the amount, and
> the date at **finanzasmiliun@gmail.com**.

### Option 2 — Bank wire / ACH transfer

Use this for larger gifts or if you prefer a bank transfer. You'll need the
details below.

| Field | Value |
|---|---|
| **Account name** | 1001 Ideas, INC |
| **Bank** | Bank of America |
| **Account number** | 229057777484 |
| **Routing number** | 063100277 |
| **Account / bank address** | 3006 Aviation Ave, Ste 2A, Coconut Grove, FL 33133 |

Notes:
- The routing number above (`063100277`) is Bank of America's **Florida
  ACH/wire** routing number. For some **international wires**, banks require
  Bank of America's SWIFT/BIC code **`BOFAUS3N`** — ask your sending bank
  which they need.
- Please add **"Venezuela earthquake relief"** as the wire reference/memo.
- Your own bank may charge a wire fee; Zelle does not. This is why Zelle is
  preferred for most donors.

---

## Tax-Deductibility — 501(c)(3) information

**1001 Ideas, INC** is organized as a **501(c)(3) tax-exempt nonprofit** under
the U.S. Internal Revenue Code.

### What this means for you as a donor

- If you are a U.S. taxpayer, your donation is **tax-deductible to the extent
  allowed by law**, provided you **itemize deductions** on your federal income
  tax return (Schedule A of Form 1040). Donors who take the standard deduction
  generally cannot deduct charitable gifts.
- You receive **no goods or services** in exchange for your donation, so the
  **full amount** is generally deductible.
- **Keep a record of every gift.** A bank/Zelle confirmation is sufficient for
  most gifts. For any **single donation of $250 or more**, the IRS requires a
  **written acknowledgment from the organization** — email us to request one.
- Deductions are claimed for the tax year in which the gift is made.

### Organization details (for your records / receipt)

| Field | Value |
|---|---|
| **Legal name** | 1001 Ideas, INC |
| **Tax status** | 501(c)(3) nonprofit |
| **EIN (Employer Identification Number)** | 32-0577080 |
| **Mailing address** | 3006 Aviation Ave, Ste 2A, Coconut Grove, FL 33133 |
| **Contact** | finanzasmiliun@gmail.com |

> ℹ️ Donors can verify an organization's tax-exempt status using the IRS
> **Tax Exempt Organization Search**: https://www.irs.gov/charities-non-profits/tax-exempt-organization-search

---

### Disclaimer

This page is provided for general informational purposes and is **not legal or
tax advice**. Tax treatment depends on your individual circumstances. Please
consult a qualified tax professional with questions about deductibility.
